...
- **状态：** Accepted
- **日期：** 2026-03-16
- **关联计划：** [HomeVital v2 开发计划 Step 1](../../.cursor/plans/homevital_v2_开发计划_a24f52a8.plan.md)
- **Supersedes：** 当前基于 `member_access_grant.can_write` 的布尔授权模型

## 背景与问题

HomeVital 已经把成员级权限作为健康数据访问的核心边界，但现有授权模型只有一个 `can_write` 布尔值，语义已经不够用：
...
问题：HomeVital 应采用什么样的授权模型，才能同时支撑成员档案访问、档案写入、AI 工具调用和授权管理界面？
