...
- `backend/app/ai` 与 `mcp-server/` 仍处于后续阶段落地前的占位状态，Phase 4 技术方向已在设计文档中收敛

## Phase 4 AI 方向

- 健康事实层继续作为 AI 的真实数据源，AI 读写通过受控工具复用现有服务层与权限模型
- Phase 4 优先实现应用内 AI 编排；MCP 继续作为 Phase 5 的对外接口层
- 推荐路线以 `PydanticAI`、`Docling`、`MinerU`、`Qwen2.5-VL`、`SenseVoice` / `FunASR`、`APScheduler` 为参考
- AI SDK、模型服务、文档解析和语音识别的具体实现，应以各项目官方文档和当前版本说明为准
